export class Question {
    static create(question) {
        return fetch('https://comments-364bb-default-rtdb.firebaseio.com/questions.json', {
            method: 'POST',
    ...
    static fetch(token) {
        if (!token) {
         ...
         return fetch(`https://comments-364bb-default-rtdb.firebaseio.com/questions.json?auth=${token}`)
            .then(response => response.json())
            .then(response => {
